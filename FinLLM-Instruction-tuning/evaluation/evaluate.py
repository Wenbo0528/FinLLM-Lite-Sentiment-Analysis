...
OUTPUT_DIR = Path(DRIVE_ROOT) / "results"  # 修改为FinLLM-Instruction-tuning下的results目录
...
class EvaluationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'label': label,
            'instruction': item['instruction']  # 添加instruction到返回值
        }
...
def load_model(self):
    ...
    self.tokenizer = AutoTokenizer.from_pretrained(
        ...
        padding_side="left",  # 确保使用left padding
        use_fast=False
    )
...
class ModelEvaluator:
    ...
    def evaluate(self):
        ...
        # 增加批处理大小
        batch_size = 32  # 对于A100来说，这个大小应该没问题
        ...
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            instructions = batch['instruction']  # 获取instructions
            labels = batch['label']  # 获取labels
            ...
            with torch.no_grad():
                outputs = self.model.generate(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    max_new_tokens=5,  # 进一步减少生成的最大token数
                    num_return_sequences=1,
                    temperature=0.7,
                    top_p=0.9,
                    do_sample=True,
