...
# RAG_CONTEXT_PATH = os.path.join(PROJECT_ROOT, "data", "rag_context_data.jsonl")
# OUTPUT_DIR = Path(__file__).parent / "results"
# OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Google Drive paths
DRIVE_ROOT = "/content/drive/MyDrive/Columbia_Semester3/5293/Final/FinLLM-Sentiment-Analysis"
INSTRUCTION_ROOT = os.path.join(DRIVE_ROOT, "FinLLM-Instruction-tuning")
RAG_ROOT = os.path.join(DRIVE_ROOT, "FinLLM-RAG")
BASE_MODEL = "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"  
...
class RAGContext:
    def __init__(self, context_path, embedding_model="all-MiniLM-L6-v2"):
    ...
    def load_contexts(self):
        logger.info("Loading RAG contexts...")
        with open(self.context_path, 'r', encoding='utf-8') as f:
            self.contexts = json.load(f)  # Load entire JSON array
        logger.info(f"Loaded {len(self.contexts)} contexts")

    def build_index(self):
        logger.info("Building FAISS index...")
        # Get embeddings for all contexts
        texts = [item['text'] for item in self.contexts]
    ...
    def overlap_coefficient(self, query, context):
        # Tokenize by whitespace and lowercase
        query_tokens = set(query.lower().split())
        context_tokens = set(context.lower().split())
        intersection = query_tokens & context_tokens
        return len(intersection) / min(len(query_tokens), len(context_tokens)) if min(len(query_tokens), len(context_tokens)) > 0 else 0
    ...
    def retrieve_context(self, query, k=3):
        ...
        for idx in indices[0]:
            ...
            sim_score = self.overlap_coefficient(query, context['text'])
            candidates.append({
                'text': context['text'],
        ...
        # Step 3: Filter by similarity > 0.8, return all that match
        filtered = [c for c in candidates if c['similarity'] > 0.8]
        return filtered
...
class InferenceDataset(Dataset):
    ...
    def __init__(self, data_path, tokenizer, rag_context, max_length=512):
    ...
    def __getitem__(self, idx):
        ...
        retrieved_contexts = self.rag_context.retrieve_context(item['instruction'])
        
        # Format context with sentiment information
        context_parts = []
        for ctx in retrieved_contexts:
            context_parts.append(f"Text: {ctx['text']}\nSentiment: {ctx['sentiment']}")
        context_text = "\n\n".join(context_parts)
...
class ModelEvaluator:
    def __init__(self, test_data_path, rag_context_path):
    ...
    def load_model(self):
        ...
        self.rag_context = RAGContext(self.rag_context_path)
        
        logger.info("Model, tokenizer, and RAG context loaded successfully")
    ...
    def evaluate(self):
        ...
        dataset = InferenceDataset(self.test_data_path, self.tokenizer, self.rag_context)
        ...
        print(f"- {OUTPUT_DIR / 'macro_micro_comparison.png'}")
        print(f"- {OUTPUT_DIR / 'error_rates.png'}")
    ...
    def plot_detailed_metrics(self, metrics, output_dir):
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(output_dir / 'macro_micro_comparison.png')
        plt.close()
...
if __name__ == "__main__":
    evaluator = ModelEvaluator(TEST_DATA_PATH, RAG_CONTEXT_PATH)
    evaluator.evaluate()
